...
describe("testLogic", () => {
  ...
  describe("chooseTestOption", () => {
    it("should return WriteCorrectAnswer when only writing is enabled", () => {
      ...
      expect(chooseTestOption(settings, 0)).toBe(TestOption.WriteCorrectAnswer);
      expect(chooseTestOption(settings, 1)).toBe(TestOption.WriteCorrectAnswer);
    });
    ...
    it("should return SelectFromMultiple when only multiSelect is enabled", () => {
      ...
      expect(chooseTestOption(settings, 0)).toBe(TestOption.SelectFromMultiple);
      expect(chooseTestOption(settings, 1)).toBe(TestOption.SelectFromMultiple);
    });
    ...
    it("should alternate strictly when both enabled and everySecondTestIsMultiOrWriting is true", () => {
      ...
      expect(chooseTestOption(settings, 0)).toBe(TestOption.WriteCorrectAnswer);
      expect(chooseTestOption(settings, 2)).toBe(TestOption.WriteCorrectAnswer);
      expect(chooseTestOption(settings, 4)).toBe(TestOption.WriteCorrectAnswer);

      // Odd indices = multi-select
      expect(chooseTestOption(settings, 1)).toBe(TestOption.SelectFromMultiple);
      expect(chooseTestOption(settings, 3)).toBe(TestOption.SelectFromMultiple);
      expect(chooseTestOption(settings, 5)).toBe(TestOption.SelectFromMultiple);
    });
  ...
  describe("checkAnswer", () => {
    ...
    it("should return 'typo' for swapped characters in longer word", () => {
      ...
      expect(checkAnswer("encyclopaedia", longWord2, "lang1to2", true)).toBe(
        "typo",
      );
    });
